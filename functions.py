...
def take_screenshot(video_path, output_path, timestamp, video_id):
    ...
    for item in timestamp:
        ...
        if not success:
            ...
            return
        cv2.imwrite(output_path + f"{video_id}--{item}.jpeg", frame)
    
    cap.release()

def generate_all_and_save(final_list):
...
def generate_prompt(current_text, previous_text, images):
    content = [
        ...
        Include in-text hyperlinks to relevant resources (e.g., You.com, Wikipedia, etc.) on any key words or phrases that might need explaining (e.g., "machine learning," "Euler Integration," etc.). Include AT LEAST 1-2 hyperlinks per paragraph.
        ...
        - {', '.join(["http://127.0.0.1:5000/{}".format(image) for image in images])}
